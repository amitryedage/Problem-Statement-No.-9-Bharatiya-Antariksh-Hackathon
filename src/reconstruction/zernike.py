...
def build_interaction_matrix(n_modes=N_ZERNIKE_MODES,
    ...
    # alpha2 = alpha * max(diag(DtD))  — standard Tikhonov scaling.
    # Previously this was squared by mistake: (alpha * max_diag)**2
    # which caused massive over-regularisation and crushed high-order modes.
    DtD    = D_mat.T @ D_mat
    alpha2 = alpha_tikhonov * np.max(np.diag(DtD) + 1e-12)
